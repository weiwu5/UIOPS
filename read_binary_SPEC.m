...
for i = 1:filenums
    ...
    while feof(fid)==0 && endfile == 0 
        ...
        % 11/3/19
        if ~isempty(varargin)
            if (fltTime(end)>=240000) && (hour<12) % Date crossover Joe Finlon 5/13/22
                hour2 = hour + 24; % hour2 is a dummy variable only for use with TAS corrections
            else
                hour2 = hour;
            end

            fltTimeInd = find(fltTime==hour2*10000+minute*100+second);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    iSlice=0;
    iii=1;
    while iii+4<=2048 % Watch for index outside of image buffer - Added by Joe Finlon - 04/17/20 %while iii<=2048
...
%                       while( bitand(buf(iii+kkk),16384)==0  && kkk<nV-2) % bin2dec('1000000000000000')
        ...
        elseif 18507==buf(iii) % 1 Hz housekeeping data
            ...
            nVPD = buf(iii-1+34);
            nHPD = buf(iii-1+35);
            vHE1 = buf(iii-1+2) * 0.00244140625; % H voltage diode 0 ~ Added Joe Finlon 4/29/22
            vHE2 = buf(iii-1+3) * 0.00244140625; % H voltage diode 64
            vHE3 = buf(iii-1+4) * 0.00244140625; % H voltage diode 127
            vVE1 = buf(iii-1+5) * 0.00244140625; % V voltage diode 0
            vVE2 = buf(iii-1+6) * 0.00244140625; % V voltage diode 64
            vVE3 = buf(iii-1+7) * 0.00244140625; % V voltage diode 127
            ...
            myformat1 = '%f, %f, %f, %d, %d, %d, %d, %d, %d, %f, %f, %f, %f, %f, %f\n';
            fid = fopen([outfilename, '.tas.csv'],'a');
            fprintf(fid, myformat1, [timehhmmss tas time nTWM nSCM nHOL nVOL nVPD nHPD vHE1 vHE2 vHE3 vVE1 vVE2 vVE3]); % Modified Joe Finlon 4/29/22
            fclose(fid);
